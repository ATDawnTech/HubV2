...
| Column | Type | Notes |
|---|---|---|
| employee_number | serial | Auto-incrementing display ID (distinct from UUID `id`) |
...
**Add:** `proficiency_label text` (Beginner/Intermediate/Advanced/Expert) for display; `verified boolean DEFAULT false`.
...
| status | text | YES | 'active' | CHECK: active, in_repair, decommissioned, unassigned |
| condition | text | YES | — | CHECK: new, good, fair, poor *(add)* |
...
Full audit trail of asset custody changes.

| Column | Type | Nullable | Default | Constraints |
|---|---|---|---|---|
| id | uuid | NO | gen_random_uuid() | PK |
| asset_id | uuid | NO | — | FK → assets.id ON DELETE CASCADE |
| assigned_from | uuid | YES | — | FK → employees.id |
